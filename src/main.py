...
class MultiPatientEnv(Wrapper):
    ...
    def __init__(self, patient_names, env_id, entry_point, max_episode_steps, reward_fun, seed=42, render_mode=None):
        ...
        self.seed = seed
        self._render_mode = render_mode  # Use _render_mode to avoid conflicts with Wrapper class
        ...
        super().__init__(env)  # Initialize the Wrapper with the first patient's environment
    ...
    def step(self, action):
        ...
        if self.steps_with_current_patient >= self.patient_switch_interval:
            logger.info(f"Forcing patient switch after {self.steps_with_current_patient} steps")
        ...
        if terminated or truncated:
            logger.info(f"Episode finished for patient: {self.patient_names[self.current_patient_idx]}")
    ...
    def reset(self, **kwargs):
        ...
        self.current_patient_idx = (self.current_patient_idx + 1) % len(self.patient_names)
        logger.info(f"Switching to patient: {self.patient_names[self.current_patient_idx]}")
        ...
        self._register_current_env()
        self.env = gymnasium.make(self.current_env_id, render_mode=self._render_mode, seed=self.seed)
    ...
    def _on_step(self):
        ...
        try:
            ...
            if self.patient_idx != current_idx:
                ...
                logger.info(f"Training on patient {patient_name} ({self.patient_idx + 1}/{self.total_patients})")
        ...
        if self.step_count % self.switch_freq == 0:
            logger.info(f"PatientSwitchCallback: Forcing patient switch after {self.switch_freq} steps")
